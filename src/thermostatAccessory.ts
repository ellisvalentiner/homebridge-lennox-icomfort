...
export class LennoxiComfortAccessory {
  ...
  constructor(
    ...
    // For local connections, use "LCC" as system ID; otherwise use system.extId
    this.systemId = this.platform.isLocalConnection ? 'LCC' : system.extId;
    // SenderID format matches app: mapp{timestamp}_{email}
    // For local connections, use appId from client; otherwise use username-based format
    if (this.platform.isLocalConnection) {
      this.senderId = this.platform.client.getAppId();
    } else {
    ...
    this.updateStatus(system).catch((error) => {
      this.platform.log.warn(
        `Failed to update initial status for ${system.extId}:`,
    ...
    try {
      ...
      if (userData.fanMode && this.fanService) {
        ...
        this.fanRotationSpeed =
          userData.fanMode === 'on' ? 100 : userData.fanMode === 'auto' ? 50 : 0;
      }
  ...
  async setTargetTemperature(value: CharacteristicValue): Promise<void> {
    try {
      ...
      this.platform.log.info(
        `[CONTROL] HomeKit requested setTargetTemperature: ${this.targetTemperature}°C`
      );
      ...
      try {
        ...
        this.platform.log.warn(
          `[CONTROL] Could not refresh system data, using cached:`,
          error instanceof Error ? error.message : String(error)
        );
      }
  ...
  async setTargetHeatingCoolingState(value: CharacteristicValue): Promise<void> {
    try {
      ...
      await this.updateSetpoints(userData.hsp, userData.csp, systemMode, userData.fanMode);
    } catch (error) {
      this.platform.log.error(
        'Error setting target heating cooling state:',
        error instanceof Error ? error.message : String(error)
      );
      throw error;
    }
  ...
  async setCoolingThresholdTemperature(value: CharacteristicValue): Promise<void> {
    try {
      ...
      this.platform.log.debug(
        `Set Cooling Threshold Temperature: ${this.coolingThresholdTemperature}°C`
      );
      ...
      const csp = isFahrenheit
        ? this.celsiusToFahrenheit(this.coolingThresholdTemperature)
        : this.coolingThresholdTemperature;

      const systemMode = this.mapOpModeToSystemMode(userData.opMode);
      await this.updateSetpoints(userData.hsp, csp.toString(), systemMode, userData.fanMode);
    } catch (error) {
      this.platform.log.error(
        'Error setting cooling threshold temperature:',
        error instanceof Error ? error.message : String(error)
      );
      throw error;
    }
  ...
  async setHeatingThresholdTemperature(value: CharacteristicValue): Promise<void> {
    try {
      ...
      this.platform.log.debug(
        `Set Heating Threshold Temperature: ${this.heatingThresholdTemperature}°C`
      );
      ...
      const hsp = isFahrenheit
        ? this.celsiusToFahrenheit(this.heatingThresholdTemperature)
        : this.heatingThresholdTemperature;

      const systemMode = this.mapOpModeToSystemMode(userData.opMode);
      await this.updateSetpoints(hsp.toString(), userData.csp, systemMode, userData.fanMode);
    } catch (error) {
      this.platform.log.error(
        'Error setting heating threshold temperature:',
        error instanceof Error ? error.message : String(error)
      );
      throw error;
    }
  ...
  async setFanRotationSpeed(value: CharacteristicValue): Promise<void> {
    try {
      ...
      this.platform.log.error(
        'Error setting fan rotation speed:',
        error instanceof Error ? error.message : String(error)
      );
      throw error;
    }
  ...
  private async updateSetpoints(
    ...
    if (this.updateInProgress) {
      ...
      if (this.updateInProgress) {
        this.platform.log.error(
          `[CONTROL] Previous update still in progress, skipping this update`
        );
        return;
      }
    }
    ...
    this.platform.log.info(
      `[CONTROL] Starting setpoint update: hsp=${hsp}, csp=${csp}, mode=${systemMode}, fanMode=${fanMode || 'auto'}`
    );
    ...
    try {
      ...
      if (isManualMode) {
        ...
        this.platform.log.info(
          `[CONTROL] Zone is in manual mode, using schedule ID ${targetScheduleId}`
        );
        ...
        this.platform.log.info(
          `[CONTROL] Zone is in override mode, using schedule ID ${targetScheduleId}`
        );
      } else {
        ...
        this.platform.log.info(
          `[CONTROL] Zone is following schedule, creating override with schedule ID ${targetScheduleId}`
        );
      }
      ...
      const spF =
        systemMode === 'heat and cool'
          ? Math.round((hspF + cspF) / 2)
          : systemMode === 'heat'
            ? Math.round(hspF)
            : Math.round(cspF);
      const spC =
        systemMode === 'heat and cool'
          ? parseFloat(((hspC + cspC) / 2).toFixed(1))
          : systemMode === 'heat'
            ? parseFloat(hspC.toFixed(1))
            : parseFloat(cspC.toFixed(1));
      ...
      this.platform.log.debug(
        `[CONTROL] Zone ID: ${zoneId}, Schedule ID: ${targetScheduleId}, StartTime: ${startTime}, NeedsHold: ${needsHold}`
      );
      ...
      this.platform.log.info(
        `[CONTROL] Publishing schedule command to schedule ID ${targetScheduleId}`
      );
      this.platform.log.debug(
        `[CONTROL] Schedule command payload: ${JSON.stringify(scheduleMessage, null, 2)}`
      );
      ...
      try {
        ...
        this.platform.log.info(
          `[CONTROL] Schedule command response: code=${scheduleResponse.code}, message="${scheduleResponse.message || ''}", retry_after=${scheduleResponse.retry_after || 0}`
        );
        this.platform.log.debug(
          `[CONTROL] Full schedule response: ${JSON.stringify(scheduleResponse, null, 2)}`
        );
      } catch (error) {
        this.platform.log.error(
          `[CONTROL] Schedule command failed with exception:`,
          error instanceof Error ? error.message : String(error)
        );
        throw error;
      }
      ...
      if (needsHold) {
        ...
        this.platform.log.info(
          `[CONTROL] Publishing zone hold command for zone ${zoneId}, schedule ${targetScheduleId}`
        );
        this.platform.log.debug(
          `[CONTROL] Zone hold command payload: ${JSON.stringify(holdMessage, null, 2)}`
        );
        ...
        try {
          ...
          this.platform.log.info(
            `[CONTROL] Zone hold command response: code=${holdResponse.code}, message="${holdResponse.message || ''}", retry_after=${holdResponse.retry_after || 0}`
          );
          this.platform.log.debug(
            `[CONTROL] Full zone hold response: ${JSON.stringify(holdResponse, null, 2)}`
          );
        } catch (error) {
          this.platform.log.error(
            `[CONTROL] Zone hold command failed with exception:`,
            error instanceof Error ? error.message : String(error)
          );
          throw error;
        }
        ...
        this.platform.log.info(
          `[CONTROL] Zone is in manual/override mode, hold command not needed`
        );
      }
      this.platform.log.info(
        `[CONTROL] Successfully updated setpoints: Heat ${hspF}°F/${hspC}°C, Cool ${cspF}°F/${cspC}°C, Mode: ${systemMode}`
      );
      ...
      this.platform.log.error(
        `[CONTROL] Error updating setpoints:`,
        error instanceof Error ? error.message : String(error)
      );
   ...
   */
  private mapOpModeToSystemMode(
    opMode: 'hc' | 'heat' | 'cool' | 'off'
  ): 'heat and cool' | 'heat' | 'cool' | 'off' {
    switch (opMode) {
      case 'hc':
