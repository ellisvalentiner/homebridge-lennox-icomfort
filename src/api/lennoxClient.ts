...
import {
  ...
  PublishMessage,
  CertificateAuthResponse,
  UserResponse,
  RegisterLCCOwnerResponse,
} from '../types';
...
export class LennoxClient {
  ...
  private thermostatIP?: string;
  private appId: string;
  private localBaseURL?: string;
  private connected: boolean = false;

  constructor(thermostatIP?: string) {
    this.isLocalConnection = !!thermostatIP;
    this.thermostatIP = thermostatIP;

    if (this.isLocalConnection) {
      // Generate unique app ID for local connections
      this.appId = `mapp${Date.now()}_user@example.com`;
      this.localBaseURL = `https://${thermostatIP}`;

      // Configure axios to accept self-signed certificates for local connections
  ...
  /**
   * Get the application ID (for local connections)
   */
  getAppId(): string {
    return this.appId;
  }
  ...
  /**
   * Check if connected (for local connections)
   */
  isConnected(): boolean {
  ...
  /**
   * Connect to thermostat (required for local connections before publishing commands)
   */
  async connect(): Promise<void> {
    if (!this.isLocalConnection || !this.thermostatIP || !this.localBaseURL) {
      throw new Error('Connect is only available for local connections');
    }

    try {
      const connectURL = `${this.localBaseURL}/Endpoints/${this.appId}/Connect`;
      ...
      if (error.response) {
        ...
        throw new Error(`Failed to connect to thermostat: ${status} ${statusText}`);
      }
      throw new Error(`Failed to connect to thermostat: ${error.message}`);
    }
  }

  /**
   * Disconnect from thermostat (for local connections)
   */
  async disconnect(): Promise<void> {
    if (!this.isLocalConnection || !this.thermostatIP || !this.localBaseURL) {
      return; // No-op for cloud connections
    }
    ...
    try {
      const disconnectURL = `${this.localBaseURL}/Endpoints/${this.appId}/Disconnect`;
      await this.api.post(
        disconnectURL,
        {},
        {
      ...
      console.warn('Failed to disconnect from thermostat:', error.message);
      this.connected = false;
    }
  }
  ...
  async authenticate(certificate?: string): Promise<CertificateAuthResponse> {
    try {
      ...
      if (error.response) {
        ...
        throw new Error(
          `Certificate authentication failed: ${status} ${statusText}${data ? ` - ${JSON.stringify(data)}` : ''}`
        );
      }
  ...
  async login(
    ...
    if (!certificateToken) {
      throw new Error(
        'Certificate token is required for login. Call authenticate() first to obtain it.'
      );
    }
    ...
    try {
      ...
      if (error.response) {
        ...
        throw new Error(
          `Login failed: ${status} ${statusText}${data ? ` - ${JSON.stringify(data)}` : ''}`
        );
      }
  ...
  async getUser(): Promise<UserResponse> {
    try {
      ...
      const response = await this.api.get<UserResponse>(`${this.baseURL}/v1/user`, {
        headers: {
      ...
      if (error.response) {
        ...
        if (status === 401) {
          throw new Error(
            `Authentication failed. Token may have expired. (${status} ${statusText})`
          );
        }
  ...
  async getSystems(plantToken?: string): Promise<LennoxSystem[]> {
    try {
      ...
      const sessionId = Math.floor(Date.now() / 1000);
      const response = await this.api.get<LennoxSystem[]>(`${this.plantBaseURL}/systems/`, {
        headers: {
  ...
  async publishCommand(
    ...
    // For local connections, ensure we're connected and use local endpoint
    if (this.isLocalConnection) {
      if (!this.connected) {
        throw new Error('Not connected to thermostat. Call connect() first.');
      }
      if (!this.localBaseURL) {
        throw new Error('Local base URL not configured');
      }

      // Ensure TargetID is "LCC" for local connections
      const localMessage = { ...message };
      if (localMessage.TargetID !== 'LCC') {
        localMessage.TargetID = 'LCC';
      }

      const endpoint = `${this.localBaseURL}/Messages/Publish`;

      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          const response = await this.api.post<PublishCommandResponse>(endpoint, localMessage, {
            headers: {
              'Content-Type': 'application/json',
              Accept: '*/*',
              'Accept-Language': 'en-US;q=1',
          ...
          if (error.response) {
            ...
            // Don't retry on 400 (bad request) or 5 (no active connection)
            if (status === 400 || (responseData && responseData.code === 5)) {
    ...
    // Cloud connection logic (existing)
    // Get current auth token from defaults (set via setAuthToken)
    const currentAuth = this.api.defaults.headers.common['Authorization'] as string | undefined;
    if (!currentAuth) {
      throw new Error('User token is required. Call login() first to obtain it.');
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        const response = await this.api.post<PublishCommandResponse>(endpoint, message, {
          headers: {
  ...
  /**
   * Request data from the system (for local connections)
   * Used for polling status updates
   *
   * @param sysId System ID (typically "LCC" for local connections)
   * @param additionalParameters JSON string with AdditionalParameters field
   * @returns Response text (JSON string for local connections)
   */
  async requestData(sysId: string, additionalParameters: string): Promise<any> {
    if (!this.isLocalConnection || !this.localBaseURL) {
      throw new Error('requestData is only available for local connections');
    }
    ...
    try {
      const url = `${this.localBaseURL}/Messages/RequestData`;
